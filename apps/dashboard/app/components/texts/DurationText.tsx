...
export default function DurationText({startDate, endDate, long}: {
    startDate: Date,
    endDate?: Date,
    long?: boolean
}) {
    ...
    const [duration, setDuration] = React.useState(() => end.getTime() - start.getTime());
    ...
    const pluralize = (value: number, singular: string, plural: string) => value === 1 ? `${value} ${singular}` : value !== 0 ? `${value} ${plural}` : "";
    ...
    return (
        <IconText icon={Clock} text={durationText}/>
    );
}