...
const ResourceBar = ({value, max, className}: { value: number, max: number, className?: string }) => {
    ...
    return (
        <div className={cn("h-1.5 w-full rounded-full bg-slate-200 dark:bg-slate-800", className)}>
            <div
        ...
        >
            ...
            >
                ...
                <div className="flex items-start gap-3 mb-3">
                    {server.icon ? (
                        <div
                            className="h-10 w-10 rounded-md bg-slate-100 dark:bg-slate-800 overflow-hidden flex-shrink-0">
                            <img src={server.icon} alt={server.name} className="h-full w-full object-cover"/>
                        </div>
                    ) : (
                        <div
                            className="flex h-10 w-10 items-center justify-center rounded-md bg-indigo-100 dark:bg-indigo-900/30 flex-shrink-0">
                ...
                <div className="mt-auto space-y-3">
                    <div className="space-y-1">
                        <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="font-medium text-slate-700 dark:text-slate-300">{server.cpuUsage}%</span>
                        </div>
                    ...
                    <div className="space-y-1">
                        <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="font-medium text-slate-700 dark:text-slate-300">
                                {formatMemory(server.memoryUsage.used)} / {formatMemory(server.memoryUsage.total)}
                            </span>
                        </div>
                        <ResourceBar value={server.memoryUsage.used} max={server.memoryUsage.total}/>
                    </div>
                </div>
