...
const BadgeStyleMap: Record<ServerStatusType, string> = {
  [ServerStatusType.UNKNOWN]: cn(
    BASE_BADGE_STYLE,
    `bg-tag-gray-background/30   text-tag-gray   border-tag-gray/80`
  ),
  [ServerStatusType.ONLINE]: cn(
    BASE_BADGE_STYLE,
    `bg-tag-green-background/30  text-tag-green  border-tag-green/50`
  ),
  [ServerStatusType.STARTING]: cn(
    BASE_BADGE_STYLE,
    `bg-tag-orange-background/30 text-tag-orange border-tag-orange/50`
  ),
  [ServerStatusType.OFFLINE]: cn(
    BASE_BADGE_STYLE,
    `bg-tag-gray-background/30   text-tag-gray   border-tag-gray/80`
  ),
  [ServerStatusType.STOPPING]: cn(
    BASE_BADGE_STYLE,
...
export default function StatusBadge({
  ...
  const badgeStyle =
    BadgeStyleMap[status] ?? BadgeStyleMap[ServerStatusType.UNKNOWN];
  const badgeDotStyle =
    BadgeDotStyleMap[status] ?? BadgeDotStyleMap[ServerStatusType.UNKNOWN];
  const statusName =
    customStatusName ?? ServerStatusTypeNames[status] ?? "Unknown";

  return (
    <Badge variant="outline" className={cn(badgeStyle, className)}>
      {status != ServerStatusType.UNKNOWN && (
        <span className={badgeDotStyle}></span>
      )}
      {statusName}
    </Badge>
  );
